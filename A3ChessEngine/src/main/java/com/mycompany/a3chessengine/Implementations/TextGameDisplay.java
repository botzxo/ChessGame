...
public class TextGameDisplay implements GameDisplay {
    ...
    final static int maxDifficulty  = 2;
    ...
    public Move promptForMove() {
        ...
        while(!(currBoard.getTile(pieceCoordX, pieceCoordY).isOccupied())){
            System.out.println("Your chosen piece is incorrect");
            
            System.out.println("Please re enter row of piece, 0 to forfiet");
            pieceCoordX = scanner.nextInt();
            if(pieceCoordX == 0){
                gameOver(-1);
                return null;
            }
            
            System.out.println("Please re enter column of piece");
            pieceCoordY = scanner.nextInt();
        }
